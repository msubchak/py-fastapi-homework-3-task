...
async def create_user(user: UserRegistrationRequestSchema, db: AsyncSession = Depends(get_db)):
    existing_user = (await db.execute(select(UserModel).where(UserModel.email == user.email))).scalar_one_or_none()
    if existing_user:
        raise HTTPException(status_code=409, detail=f"A user with this email {user.email} already exists.")

    user_group = (await db.execute(select(UserGroupModel).where(UserGroupModel.name == UserGroupEnum.USER))).scalar_one_or_none()
    if not user_group:
...
async def activate_user(payload: UserActivationRequestSchema, db: AsyncSession = Depends(get_db)):
    activation_token = (
        await db.execute(
            ...
            .join(UserModel)
            .where(UserModel.email == payload.email, ActivationTokenModel.token == payload.token)
        )
    ).scalar_one_or_none()
    ...
    return JSONResponse(status_code=status.HTTP_200_OK, content={"message": "User account activated successfully."})
...
async def reset_password(payload: PasswordResetRequestSchema, db: AsyncSession = Depends(get_db)):
    ...
    if user and user.is_active:
        tokens = (await db.execute(select(PasswordResetTokenModel).where(PasswordResetTokenModel.user_id == user.id))).scalars().all()
        for token in tokens:
            db.delete(token)
...
async def login(payload: UserLoginRequestSchema, db: AsyncSession = Depends(get_db), jwt_manager: JWTAuthManagerInterface = Depends(get_jwt_auth_manager)):
    user = (await db.execute(select(UserModel).where(UserModel.email == payload.email))).scalar_one_or_none()
    ...
    return UserLoginResponseSchema(access_token=access_token, refresh_token=refresh_token.token, token_type="bearer")

@router.post("/refresh/", response_model=TokenRefreshResponseSchema, status_code=status.HTTP_200_OK)
async def refresh_token(payload: TokenRefreshRequestSchema, db: AsyncSession = Depends(get_db), jwt_manager: JWTAuthManagerInterface = Depends(get_jwt_auth_manager)):
    try:
    ...
    refresh_token = (await db.execute(select(RefreshTokenModel).where(RefreshTokenModel.token == payload.refresh_token))).scalar_one_or_none()
    if not refresh_token:
        raise HTTPException(status_code=401, detail="Refresh token not found.")

    user = (await db.execute(select(UserModel).where(UserModel.id == refresh_token.user_id))).scalar_one_or_none()
    if not user:
        raise HTTPException(status_code=404, detail="User not found.")

    access_token = jwt_manager.create_access_token(data={"user_id": user_id})
    return TokenRefreshResponseSchema(access_token=access_token, token_type="bearer")
