...
async def create_user(
    ...
    try:
        ...
        activate_token = ActivationTokenModel(
            user_id=db_user.id,
            expires_at=datetime.now(timezone.utc).replace(tzinfo=timezone.utc) + timedelta(hours=24)
        )
        db.add(activate_token)
        await db.commit()
        await db.refresh(activate_token)
...
async def reset_password(
    ...
    if user and user.is_active:
        tokens = await db.execute(select(PasswordResetTokenModel).where(PasswordResetTokenModel.user_id == user.id))
        for token in tokens.scalars().all():
            await db.delete(token)

        new_token = PasswordResetTokenModel(
            user_id=user.id,
            expires_at=datetime.now(timezone.utc).replace(tzinfo=timezone.utc) + timedelta(hours=1)
        )
        db.add(new_token)
        await db.commit()
...
async def refresh_token(
    ...
    refresh_token = db_token.scalar_one_or_none()
    if not refresh_token:
        raise HTTPException(status_code=401, detail="Refresh token not found.")

    result = await db.execute(select(UserModel).where(UserModel.id == refresh_token.user_id))
    user = result.scalar_one_or_none()
    if not user:
